...
testRatio = 0.2  # 20% of the images will be used for a testing  | 80% for a training
validationRatio = 0.2  # from all the images for a training, program will take 20% of that for a validation
###################################################
...
print("Total Classes Detected:", len(myList)) # 43
...
X_train = np.array(list(map(preprocessing, X_train)))  # TO ITERATATE AND PREPROCESS ALL IMAGES
...
zdjecie = X_train[30]
zdjecie = cv2.resize(zdjecie, (300, 300))
cv2.imshow("1 bit",zdjecie)
...
############################### CONVOLUTION NEURAL NETWORK MODEL LeNET
def myModel():
    no_Of_Filters = 60
